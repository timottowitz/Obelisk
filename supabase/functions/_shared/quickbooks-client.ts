...
const QUICKBOOKS_API_URL_PROD = "https://quickbooks.api.intuit.com";
const QUICKBOOKS_API_URL_SANDBOX = "https://sandbox-quickbooks.api.intuit.com";
...
export class QuickBooksClient {
  ...
  private getApiBaseUrl(): string {
    if (this.connection?.is_sandbox === true) {
      return QUICKBOOKS_API_URL_SANDBOX;
    }
    if (this.connection?.is_sandbox === false) {
      return QUICKBOOKS_API_URL_PROD;
    }
    // Fallback to environment if flag is missing
    return QUICKBOOKS_ENVIRONMENT === "production"
      ? QUICKBOOKS_API_URL_PROD
  ...
  private async makeRequest(
    ...
    const url = `${baseUrl}/v3/company/${this.connection.realm_id}${endpoint}${separator}minorversion=${API_MINOR_VERSION}`;
  ...
  // Customer methods
  createCustomer(customer: QuickBooksCustomer): Promise<any> {
  ...
  queryCustomers(query: string): Promise<any> {
    const encodedQuery = encodeURIComponent(query);
    return this.makeRequest("GET", `/query?query=${encodedQuery}`);
  }

  // Purchase methods
  createPurchase(purchase: QuickBooksPurchase): Promise<any> {
    return this.makeRequest("POST", "/purchase", purchase);
  }

  getPurchase(purchaseId: string): Promise<any> {
    return this.makeRequest("GET", `/purchase/${purchaseId}`);
  }

  // Bill methods
  createBill(bill: QuickBooksBill): Promise<any> {
  ...
  queryVendors(query: string): Promise<any> {
    const encodedQuery = encodeURIComponent(query);
    return this.makeRequest("GET", `/query?query=${encodedQuery}`);
  }
