// Default case types and folder templates configuration
export interface FolderTemplate {
  name: string;
  path: string;
  parent_path?: string;
  sort_order: number;
  is_required?: boolean;
  children?: FolderTemplate[];
}

export interface CaseTypeConfig {
  name: string;
  display_name: string;
  description: string;
  color: string;
  icon: string;
  folder_templates: FolderTemplate[];
}

export interface ContactType {
  name: string;
}

export interface ExpenseType {
  name: string;
}

export interface CostType {
  name: string;
}

export const DEFAULT_EXPENSE_TYPES: ExpenseType[] = [
  { name: "Bill" },
  { name: "Check" },
  { name: "Credit Card" },
  { name: "Soft Costs" },
];

export const DEFAULT_CASE_TYPES: CaseTypeConfig[] = [
  {
    name: "solar",
    display_name: "Solar",
    description: "Solar matters and consultations",
    color: "#3B82F6",
    icon: "sun",
    folder_templates: [
      {
        name: "Expenses",
        path: "/expenses",
        sort_order: 1,
      },
      {
        name: "Insurance",
        path: "/insurance",
        sort_order: 2,
      },
      {
        name: "Roof Expert Report",
        path: "/roof-expert-report",
        sort_order: 3,
      },
      {
        name: "Solar Expert Report",
        path: "/solar-expert-report",
        sort_order: 4,
      },
      {
        name: "UCC1 Filings and Lien Notices",
        path: "/ucc1-filings-and-lien-notices",
        sort_order: 5,
      },
      {
        name: "Solar Contracts",
        path: "/solar-contracts",
        sort_order: 6,
      },
      {
        name: "Settlement",
        path: "/settlement",
        sort_order: 7,
      },
      {
        name: "Pre-Panel Electricity Bills",
        path: "/pre-panel-electricity-bills",
        sort_order: 8,
      },
      {
        name: "Post-Panel Electricity Bills",
        path: "/post-panel-electricity-bills",
        sort_order: 9,
      },
      {
        name: "Photos of Home and Panels",
        path: "/photos-of-home-and-panels",
        sort_order: 10,
      },
      {
        name: "Installers or Subcontractors Docs",
        path: "/installers-or-subcontractors-docs",
        sort_order: 11,
      },
      {
        name: "Financing Contracts",
        path: "/financing-contracts",
        sort_order: 12,
      },
      {
        name: "Emails re Contracts",
        path: "/emails-re-contracts",
        sort_order: 13,
      },
      {
        name: "Bennett Legal Communications",
        path: "/bennett-legal-communications",
        sort_order: 14,
      },
      {
        name: "Banking Records",
        path: "/banking-records",
        sort_order: 15,
      },
      {
        name: "Arbitration",
        path: "/arbitration",
        sort_order: 16,
      },
      {
        name: "Administrative",
        path: "/administrative",
        sort_order: 17,
      },
    ],
  },
  {
    name: "litigation",
    display_name: "Litigation",
    description: "Litigation matters and consultations",
    color: "#000000",
    icon: "case",
    folder_templates: [
      {
        name: "Administrative",
        path: "/administrative",
        sort_order: 18,
      },
      {
        name: "Correspondence",
        path: "/correspondence",
        sort_order: 19,
        children: [
          {
            name: "Drafts",
            path: "/correspondence/drafts",
            parent_path: "/correspondence",
            sort_order: 20,
          },
        ],
      },
      {
        name: "Demands",
        path: "/demands",
        sort_order: 21,
        children: [
          {
            name: "Drafts",
            path: "/demands/drafts",
            parent_path: "/demands",
            sort_order: 22,
          },
        ],
      },
      {
        name: "Depo Prep",
        path: "/depo-prep",
        sort_order: 23,
      },
      {
        name: "Depositions",
        path: "/depositions",
        sort_order: 24,
      },
      {
        name: "Discovery",
        path: "/discovery",
        sort_order: 25,
        children: [
          {
            name: "Defendant Produced",
            path: "/discovery/defendant-produced",
            parent_path: "/discovery",
            sort_order: 26,
            children: [
              {
                name: "Drafts",
                path: "/discovery/defendant-produced/drafts",
                parent_path: "/discovery/defendant-produced",
                sort_order: 27,
              },
              {
                name: "Plaintiff Produced",
                path: "/discovery/defendant-produced/plaintiff-produced",
                parent_path: "/discovery/defendant-produced",
                sort_order: 28,
              },
            ],
          },
        ],
      },
      {
        name: "Expenses",
        path: "/expenses",
        sort_order: 29,
      },
      {
        name: "Experts",
        path: "/experts",
        sort_order: 30,
        children: [
          {
            name: "Defendant Experts",
            path: "/experts/defendant-experts",
            parent_path: "/experts",
            sort_order: 31,
            children: [
              {
                name: "Plaintiff Experts",
                path: "/experts/defendant-experts/plaintiff-experts",
                parent_path: "/experts/defendant-experts",
                sort_order: 32,
              },
            ],
          },
        ],
      },
      {
        name: "Insurance",
        path: "/insurance",
        sort_order: 33,
        children: [
          {
            name: "Correspondence",
            path: "/insurance/correspondence",
            parent_path: "/insurance",
            sort_order: 34,
          },
        ],
      },
      {
        name: "Work Product",
        path: "/work-product",
        sort_order: 35,
      },
      {
        name: "Lop and Liens",
        path: "/lop-and-liens",
        sort_order: 36,
        children: [
          {
            name: "Drafts",
            path: "/lop-and-liens/drafts",
            parent_path: "/lop-and-liens",
            sort_order: 37,
            children: [
              {
                name: "Correspondence",
                path: "/lop-and-liens/drafts/correspondence",
                parent_path: "/lop-and-liens/drafts",
                sort_order: 38,
              },
            ],
          },
        ],
      },
      {
        name: "Mediation",
        path: "/mediation",
        sort_order: 39,
        children: [
          {
            name: "Correspondence",
            path: "/mediation/correspondence",
            parent_path: "/mediation",
            sort_order: 40,
          },
        ],
      },
      {
        name: "Medical",
        path: "/medical",
        sort_order: 41,
        children: [
          {
            name: "Native",
            path: "/medical/native",
            parent_path: "/medical",
            sort_order: 42,
            children: [
              {
                name: "Request",
                path: "/medical/native/request",
                parent_path: "/medical/native",
                sort_order: 43,
              },
            ],
          },
        ],
      },
      {
        name: "Photos",
        path: "/photos",
        sort_order: 44,
      },
      {
        name: "Pleadings",
        path: "/pleadings",
        sort_order: 45,
        children: [
          {
            name: "Drafts",
            path: "/pleadings/drafts",
            parent_path: "/pleadings",
            sort_order: 46,
          },
        ],
      },
      {
        name: "Settlement",
        path: "/settlement",
        sort_order: 47,
        children: [
          {
            name: "Correspondence",
            path: "/settlement/correspondence",
            parent_path: "/settlement",
            sort_order: 48,
            children: [
              {
                name: "Disbursement",
                path: "/settlement/correspondence/disbursement",
                parent_path: "/settlement/correspondence",
                sort_order: 49,
              },
              {
                name: "Agreements",
                path: "/settlement/correspondence/agreements",
                parent_path: "/settlement/correspondence",
                sort_order: 50,
              },
              {
                name: "Offers",
                path: "/settlement/correspondence/offers",
                parent_path: "/settlement/correspondence",
                sort_order: 51,
              },
            ],
          },
        ],
      },
      {
        name: "Trial",
        path: "/trial",
        sort_order: 52,
      },
    ],
  },
  {
    name: "IMVA",
    display_name: "IMVA",
    description: "IMVA matters and consultations",
    color: "#10B981",
    icon: "building",
    folder_templates: [
      {
        name: "Medicals",
        path: "/medicals",
        sort_order: 53,
      },
      {
        name: "Mediation",
        path: "/mediation",
        sort_order: 54,
      },
      {
        name: "Pleadings",
        path: "/pleadings",
        sort_order: 55,
      },
      {
        name: "Lop and Liens",
        path: "/lop-and-liens",
        sort_order: 56,
      },
      {
        name: "Investigation",
        path: "/investigation",
        sort_order: 57,
      },
      {
        name: "Experts",
        path: "/experts",
        sort_order: 58,
      },
      {
        name: "Expenses",
        path: "/expenses",
        sort_order: 59,
      },
      {
        name: "Discovery",
        path: "/discovery",
        sort_order: 60,
      },
      {
        name: "Depositions",
        path: "/depositions",
        sort_order: 61,
      },
      {
        name: "Depo Prep",
        path: "/depo-prep",
        sort_order: 62,
      },
      {
        name: "Correspondence",
        path: "/correspondence",
        sort_order: 63,
      },
      {
        name: "Administrative",
        path: "/administrative",
        sort_order: 64,
      },
    ],
  },
];

export const DEFAULT_CONTACT_TYPES: ContactType[] = [
  { name: "Adjuster" },
  { name: "Attorney" },
  { name: "Business" },
  { name: "Client" },
  { name: "Court" },
  { name: "Defendant" },
  { name: "Emergency Department" },
  { name: "Expert" },
  { name: "Firm" },
  { name: "Insurance Company" },
  { name: "Involved Party" },
  { name: "Judge" },
  { name: "Mediator" },
  { name: "Medical Provider" },
  { name: "Person" },
  { name: "Plantiff" },
  { name: "PNC" },
  { name: "Staff" },
];

export const DEFAULT_COST_TYPES: CostType[] = [
  { name: "Arbitrator's Fees" },
  { name: "Attorney's Fees" },
  { name: "Certified Crash Report" },
  { name: "Court Filing Fee" },
  { name: "Demand Letter Drafting" },
  { name: "Deposition Transcript" },
  { name: "Expert Fee" },
  { name: "Flight" },
  { name: "Focus Group/Mock Trial" },
  { name: "Gas" },
  { name: "Hotel" },
  { name: "Investigation" },
  { name: "Mailing Service" },
  { name: "Meal" },
  { name: "Mediator Fees" },
  { name: "Mefical Record" },
  { name: "Medical Report" },
  { name: "Notary" },
  { name: "Open Records" },
  { name: "Phone Conferencing" },
  { name: "Postage" },
  { name: "Service of Process" },
  { name: "Taxi/Uber" },
];

// Function to seed case types for a specific schema
export async function seedCaseTypes(
  supabase: any,
  schema: string
): Promise<void> {
  console.log(`Seeding case types for schema: ${schema}`);

  for (const caseTypeConfig of DEFAULT_CASE_TYPES) {
    try {
      // Check if case type already exists
      const { data: existingCaseType } = await supabase
        .schema(schema)
        .from("case_types")
        .select("id")
        .eq("name", caseTypeConfig.name)
        .single();

      let caseTypeId: string;

      if (existingCaseType) {
        console.log(
          `Case type ${caseTypeConfig.name} already exists, skipping...`
        );
        caseTypeId = existingCaseType.id;
      } else {
        // Create case type
        const { data: newCaseType, error: caseTypeError } = await supabase
          .schema(schema)
          .from("case_types")
          .insert({
            name: caseTypeConfig.name,
            display_name: caseTypeConfig.display_name,
            description: caseTypeConfig.description,
            color: caseTypeConfig.color,
            icon: caseTypeConfig.icon,
          })
          .select("id")
          .single();

        if (caseTypeError) {
          console.error(
            `Failed to create case type ${caseTypeConfig.name}:`,
            caseTypeError
          );
          continue;
        }

        caseTypeId = newCaseType.id;
        console.log(`Created case type: ${caseTypeConfig.display_name}`);
      }

      // Create folder templates
      for (const template of caseTypeConfig.folder_templates) {
        try {
          // Check if template already exists
          const { data: existingTemplate } = await supabase
            .schema(schema)
            .from("folder_templates")
            .select("id")
            .eq("case_type_id", caseTypeId)
            .eq("path", template.path)
            .single();

          if (existingTemplate) {
            console.log(
              `Template ${template.name} already exists for ${caseTypeConfig.name}, skipping...`
            );
            continue;
          }

          // Create template
          await createFolderTemplate(supabase, caseTypeId, template, schema);

          console.log(`  Created template: ${template.name}`);
        } catch (error) {
          console.error(`Error creating template ${template.name}:`, error);
        }
      }
    } catch (error) {
      console.error(
        `Error processing case type ${caseTypeConfig.name}:`,
        error
      );
    } finally {
      console.log(`Finished seeding case types for schema: ${schema}`);
    }
  }
}

export async function seedContactTypes(
  supabase: any,
  schema: string
): Promise<void> {
  console.log(`Seeding contact types for schema: ${schema}`);

  for (const contactType of DEFAULT_CONTACT_TYPES) {
    try {
      // Check if contact type already exists
      const { data: existingContactType } = await supabase
        .schema(schema)
        .from("contact_types")
        .select("id")
        .eq("name", contactType.name)
        .single();

      if (existingContactType) {
        console.log(
          `Contact type ${contactType.name} already exists, skipping...`
        );
        continue;
      }

      const { error: contactTypeError } = await supabase
        .schema(schema)
        .from("contact_types")
        .insert({
          name: contactType.name,
        });

      if (contactTypeError) {
        console.error(
          `Failed to create contact type ${contactType.name}:`,
          contactTypeError
        );
        continue;
      }
    } catch (error) {
      console.error(
        `Error processing contact type ${contactType.name}:`,
        error
      );
    } finally {
      console.log(`Finished seeding contact types for schema: ${schema}`);
    }
  }
}

export async function seedCostTypes(
  supabase: any,
  schema: string
): Promise<void> {
  console.log(`Seeding cost types for schema: ${schema}`);

  for (const costType of DEFAULT_COST_TYPES) {
    try {
      // Check if cost type already exists
      const { data: existingCostType } = await supabase
        .schema(schema)
        .from("cost_types")
        .select("id")
        .eq("name", costType.name)
        .single();

      if (existingCostType) {
        console.log(`Cost type ${costType.name} already exists, skipping...`);
        continue;
      }

      const { error: costTypeError } = await supabase
        .schema(schema)
        .from("cost_types")
        .insert({
          name: costType.name,
        });

      if (costTypeError) {
        console.error(
          `Failed to create cost type ${costType.name}:`,
          costTypeError
        );
        continue;
      }
    } catch (error) {
      console.error(`Error processing cost type ${costType.name}:`, error);
    } finally {
      console.log(`Finished seeding cost types for schema: ${schema}`);
    }
  }
}

export async function seedExpenseTypes(
  supabase: any,
  schema: string
): Promise<void> {
  console.log(`Seeding expense types for schema: ${schema}`);

  for (const expenseType of DEFAULT_EXPENSE_TYPES) {
    try {
      // Check if expense type already exists
      const { data: existingExpenseType } = await supabase
        .schema(schema)
        .from("expense_types")
        .select("id")
        .eq("name", expenseType.name)
        .single();

      if (existingExpenseType) {
        console.log(
          `Expense type ${expenseType.name} already exists, skipping...`
        );
        continue;
      }

      const { error: expenseTypeError } = await supabase
        .schema(schema)
        .from("expense_types")
        .insert({
          name: expenseType.name,
        });

      if (expenseTypeError) {
        console.error(
          `Failed to create expense type ${expenseType.name}:`,
          expenseTypeError
        );
        continue;
      }
    } catch (error) {
      console.error(
        `Error processing expense type ${expenseType.name}:`,
        error
      );
    } finally {
      console.log(`Finished seeding expense types for schema: ${schema}`);
    }
  }
}

async function createFolderTemplate(
  supabase: any,
  caseTypeId: string,
  template: FolderTemplate,
  schema: string
) {
  const { error: templateError } = await supabase
    .schema(schema)
    .from("folder_templates")
    .insert({
      case_type_id: caseTypeId,
      name: template.name,
      path: template.path,
      parent_path: template.parent_path || null,
      sort_order: template.sort_order,
      is_required: template.is_required !== false,
    });

  if (templateError) {
    console.error(`Failed to create template ${template.name}:`, templateError);
  }

  if (template.children) {
    for (const child of template.children) {
      await createFolderTemplate(supabase, caseTypeId, child, schema);
    }
  }
}
